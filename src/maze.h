...
class SquareMaze
{
  public:
    ...
    void solveHelper(std::vector<std::vector<std::pair<int, int>>> & traverse, std::pair<int, int> start_point);

    // void solveMazeHelper(std::vector<std::vector<std::pair<int, int>>> * solution, std::pair<unsigned, unsigned> start);
  ...
  private:
    int width_;
    int height_;
    DisjointSets disjoint_;

    struct WallData {
      bool right_;
      bool down_;
    };
    std::vector<WallData> maze_;
};